...
test('Visitor log CRUD and statistics workflow', async () => {
  ...
  try {
    ...
    const v1 = await visitors.create({
    ...
    const v2 = await visitors.create({
    ...
    const searchRes = await visitors.all({ search: 'Santos' });
    assert.ok(searchRes.some(v => v.id === v1.id), 'Search by visitor name should return v1');

    // 3. Query with group filter
    const schoolRes = await visitors.all({ groupType: 'School Tour' });
    ...
    const updated = await visitors.update(v1.id, {
      status: 'Completed',
    ...
    // 5. Test stats
    const statData = await visitors.stats();
    ...
    const rem1 = await visitors.remove(v1.id);
    const rem2 = await visitors.remove(v2.id);
    assert.equal(rem1, true, 'Remove v1 should succeed');
    assert.equal(rem2, true, 'Remove v2 should succeed');
    assert.equal(await visitors.findById(v1.id), null, 'v1 should not be found');
  } finally {
...
test('Artifacts log CRUD, auto accession numbering, and statistics workflow', async () => {
  ...
  try {
    ...
    const a1 = await artifactLogs.create({
    ...
    const a2 = await artifactLogs.create({
    ...
    // 5. Test stats
    const statData = await artifactLogs.stats();
    ...
    const rem1 = await artifactLogs.remove(a1.id);
    const rem2 = await artifactLogs.remove(a2.id);
    assert.equal(rem1, true, 'Remove a1 should succeed');
    assert.equal(rem2, true, 'Remove a2 should succeed');
    assert.equal(await artifactLogs.findById(a1.id), null, 'a1 should not be found');
  } finally {
    await mongoose.disconnect();
    await mongoServer.stop();
  }
